-- ============================================================================
-- DEALFLOW ANALYTICS - DIRECT APPLICATION SCRIPT
-- ============================================================================
-- Apply this script directly in Supabase SQL Editor
-- This script drops and recreates all analytics views and functions
-- ============================================================================
...
DROP VIEW IF EXISTS ic_member_performance CASCADE;
DROP VIEW IF EXISTS manager_productivity CASCADE;
DROP VIEW IF EXISTS monthly_dealflow_trends CASCADE;
DROP VIEW IF EXISTS investor_interest_analytics CASCADE;

-- Drop existing functions
DROP FUNCTION IF EXISTS get_dealflow_waterfall(DATE, DATE) CASCADE;
DROP FUNCTION IF EXISTS get_dealflow_kpis(DATE, DATE) CASCADE;

-- ----------------------------------------------------------------------------
-- 1. DEALFLOW WATERFALL STATISTICS VIEW
-- ----------------------------------------------------------------------------
CREATE VIEW dealflow_waterfall_stats AS
WITH deal_stages AS (
  SELECT
...
-- ----------------------------------------------------------------------------
CREATE VIEW deal_velocity_metrics AS
SELECT
  d.id AS deal_id,
  d.status,
...
-- ----------------------------------------------------------------------------
CREATE VIEW ic_member_performance AS
SELECT
...
-- ----------------------------------------------------------------------------
CREATE VIEW manager_productivity AS
SELECT
  p.id AS manager_id,
...
-- ----------------------------------------------------------------------------
-- 6. MONTHLY DEALFLOW TRENDS VIEW (FIXED)
-- ----------------------------------------------------------------------------
CREATE VIEW monthly_dealflow_trends AS
...
-- ----------------------------------------------------------------------------
CREATE VIEW investor_interest_analytics AS
SELECT
  p.id AS investor_id,
...
-- ----------------------------------------------------------------------------
CREATE FUNCTION get_dealflow_waterfall(
...
-- ----------------------------------------------------------------------------
CREATE FUNCTION get_dealflow_kpis(
  start_date DATE DEFAULT NULL,
  end_date DATE DEFAULT NULL
)
RETURNS TABLE (
...
-- ============================================================================
-- All views, functions, and indexes have been created successfully.
-- You can now query the analytics views or navigate to /app/analytics
-- ============================================================================
