...
export default function DealflowManagerNewDeal() {
  ...
  useEffect(() => {
    async function checkRole() {
      ...
      if (!profile || !["dealflow_manager", "admin"].includes(profile.role)) {
        alert("Access denied. Only dealflow managers can create deals.");
        router.push("/app");
        return;
      }
  ...
  async function createDeal() {
    try {
      ...
      // Create startup first (dealflow manager creates on behalf of founder)
      const { data: startup, error: startupError } = await supabase
        ...
        .insert({
          ...
          team_summary: startupForm.team_summary || null,
          traction_summary: startupForm.traction_summary || null,
          owner_user_id: null, // No owner (dealflow manager uploaded)
          created_by: uid,
        })
        .select("*")
        .single();
      ...
      const { data: deal, error: dealError } = await supabase
        ...
        .insert({
          ...
          created_by: uid,
          status: "screening_in_progress", // Dealflow manager deals go straight to screening
      ...
      if (pitchDeck) {
        ...
        if (uploadError) throw new Error("Failed to upload pitch deck: " + uploadError.message);
  ...
  if (!role) {
    return <div className="text-sm text-black/60">Checking permissions...</div>;
  }

  return (
    <div className="mx-auto max-w-3xl space-y-4">
      <div>
        <div className="text-2xl font-semibold">Create new deal</div>
        <div className="text-sm text-black/60">
          Create a deal submission received 1:1. Deal will be flagged as dealflow manager uploaded.
        </div>
      </div>

      <Card>
        <CardHeader>
          <div className="text-sm font-semibold">Startup information</div>
          <div className="text-xs text-black/60">Enter details about the startup submitting this deal.</div>
        </CardHeader>
        <CardContent>
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <div className="md:col-span-2">
              <Textarea
                placeholder="Team summary (founders, key hires)"
                rows={2}
                value={startupForm.team_summary}
                onChange={(e) => setStartupForm({ ...startupForm, team_summary: e.target.value })}
              />
            </div>
            <div className="md:col-span-2">
              <Textarea
                placeholder="Traction summary (revenue, users, growth)"
                rows={2}
                value={startupForm.traction_summary}
                onChange={(e) => setStartupForm({ ...startupForm, traction_summary: e.target.value })}
              />
            </div>
          </div>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <div className="text-xs text-black/60">Upload pitch deck PDF (max 10MB, required for AI screening).</div>
        </CardHeader>
        <CardContent>
      ...
      <div className="flex items-center justify-between">
        <div className="text-xs text-black/50">
          💡 Deal will be created with status "screening" and flagged as dealflow manager uploaded
        </div>
        <Button onClick={createDeal} disabled={!startupForm.name || uploading}>
