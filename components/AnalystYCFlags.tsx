...
export function AnalystYCFlags({ dealId, onSaved }: { dealId: string; onSaved: () => Promise<void> }) {
  ...
  async function aiDetectFlags() {
    ...
    try {
      ...
      console.log("Calling AI Detect Flags:", url);
      ...
      console.log("AI Detect Flags response status:", res.status);

      if (!res.ok) {
        const data = await res.json().catch(() => ({ error: "Unknown error" }));
        console.error("AI Detect Flags error:", data);
        throw new Error(data.error || `AI flag detection failed with status ${res.status}`);
      }

      const data = await res.json();
      console.log("AI Detect Flags success:", data);
      ...
      alert(`AI flag detection failed: ${e.message || String(e)}`);
    } finally {
      setAiScanning(false);
    }
  }
  ...
  async function save() {
    ...
    try {
      ...
      const { error } = await supabase.from("analyst_deal_flags").upsert(
        {
          deal_id: dealId,
          analyst_user_id: uid,
